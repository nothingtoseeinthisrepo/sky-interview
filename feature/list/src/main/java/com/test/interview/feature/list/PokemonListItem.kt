...
class PokemonListItemView @JvmOverloads constructor(
    ...
    fun setPokemon(pokemon: Pokemon, onClick: (Pokemon) -> Unit) {
        ...
        foreground = pokemon.pokemonClickRipple(context)
        binding.root.setOnClickListener { onClick(pokemon) }
    }
}

private fun Pokemon.pokemonClickRipple(context: Context): RippleDrawable? {
    ...
    val rippleColor = types.firstOrNull()?.highlightColor?.copy(alpha = .5f)
    rippleColor?.let {
